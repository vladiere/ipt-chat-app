<template>
  <div class="column q-pa-md shadow-4 rounded-borders">
    <div class="row justify-between">
      <span class="text-h6 text-primary">Chats</span>
    </div>
...
const getOnlineUsers = debounce(() => {
  socket.on('user_online', (data) => {
    onlineUsers.value = data;
  });
}, 1500)

const getRecentMessages = debounce(async () => {
  try {
    const response = await api.post('/user/get/recent', { uuid: decode.uuid }, {
      headers: {
